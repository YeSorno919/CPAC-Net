...
def train(snapshot_path,zhibiao_dir,fold_num,data_file,epo_num=500,baifenbi=1,confidence_threshold=0.95):#bili用指百分之多少标注数据
    ...
    summary_info = f"最终峰值显存: {peak_memory:.4f}MB" #20260330发现此前代码写成了GB，在之前的历史保留文件中，注意转换为MB
    print(summary_info)
    logging.info(summary_info)
...
def main():
    ...
    snapshot_path=f'/mnt/hys/CPAC_Net/My_mt_with_reg_strong_weak_outputs/25%_lr_1e-3_500epoch_置信度阈值95e-2_20250412_consis_1e-2-2e-2_CE_s_w_threshold_6e-1_4e-1_变形图像也参与训练0.5'
    ...
    source1 = '/mnt/hys/CPAC_Net/train_my_mt_with_reg_同个病人_strong_weak.py'
