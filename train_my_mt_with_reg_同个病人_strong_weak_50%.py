            ...
            if iter_num%len(trainloader)==0:
                ...
                with torch.no_grad():
                    ...
                    if avg_iou_test2 > best_performance2: #保存测试集合上最好的模型
                        ...
                        logging.info(f"计算并记录达到集上达到最优! Epoch: {best_performance_epoch}, 总训练耗时: {best_performance_total_time:.4f}秒")
...
def main(bili_str,baifenbi):
    ...
    snapshot_path=f'/mnt/hys/CPAC_Net/My_mt_with_reg_strong_weak_outputs_50%/20251225_lr_1e-3_500epoch_置信度阈值95e-2_consis_0-1e-1_KL_s_w_threshold_6e-1_4e-1_变形图像也参与训练0.5_aug2_testbest'
    
    if not os.path.exists(snapshot_path):
        print("%s没有，正在创建" % snapshot_path)
        os.makedirs(snapshot_path)
    source1 = '/mnt/hys/CPAC_Net/train_my_mt_with_reg_同个病人_strong_weak_50%.py'
