"""
    ...
    33%是由于同患者4张图像有3张用于训练（其中1张有标签，2张无标签），1张用于测试.
    双源互补+变形图像补充监督的实验 少了强弱增强一致性
...
from calzhibiao import cal_zhibiao,cal_zhibiao_hys_dantongdao,cal_zhibiao_hys_dantongdao_teshubiaozhunhua_RCPS,cal_zhibiao_hys_dantongdao_jiandanguiyihua_RCPS,cal_zhibiao_hys_dantongdao_jiandanguiyihua_RCPS_33percent
...
def train(snapshot_path,zhibiao_dir,fold_num,data_file,epo_num=200,baifenbi=1,confidence_threshold=0.95):#bili用指百分之多少标注数据
    ...
    db_train= hysDataSets_dantongdao_jiandanguiyihua_33percent(data_file=data_file, img_path="/mnt/hys/Datasets/重复测量（hys整理版）/img_crop_npy", zhibiao_dir=zhibiao_dir,
                            split="train",baifenbi=baifenbi,with_affine_field=True)
    db_test = hysDataSets_dantongdao_jiandanguiyihua_33percent(data_file=data_file, img_path="/mnt/hys/Datasets/重复测量（hys整理版）/img_crop_npy", zhibiao_dir=zhibiao_dir,
                          split="test")
    db_val = hysDataSets_dantongdao_jiandanguiyihua_33percent(data_file=data_file, img_path="/mnt/hys/Datasets/重复测量（hys整理版）/img_crop_npy", zhibiao_dir=zhibiao_dir,
                          split="val")
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            with autocast():
                ...
                # if(1)
                    ...
                    # pseudo_name = unlabelname[0]
                    # 遍历 pseudo_all_metrics 中的每个元素
                    have_save =True
                    # for metric in pseudo_all_metrics:
                    #     image_name = metric['image_name']
                    #     if image_name == pseudo_name:
                    #         have_save = True
                    #         break
                    if have_save ==False:
                        ...
                        # GT_gland= unlabel_img_label_for_compair[0]
                        # GT_bg = 1-unlabel_img_label_for_compair[0]
                        ...
                        #     #计算召回率和准确率
                        #     # 补充前：
                        # TP_before = np.logical_and(gland_pred_certain, GT_gland).sum()
                        # TN_before = np.logical_and(bg_pred_certain, GT_bg).sum()
                        # total_pseudo_bf = certain_mask.sum ()
                        # Recall_before = (TP_before+TN_before) / GT_gland.size
                        # Acc_before = (TP_before+TN_before) / total_pseudo_bf if total_pseudo_bf != 0 else 0
                            
                        #     # 补充后：
                        # TP_after = np.logical_and(gland_generate_for_sup[0,0], GT_gland).sum()
                        # TN_after = np.logical_and(bg_generate_for_sup[0,0], GT_bg).sum()
                        # total_pseudo_af = generate_mask.sum ()
                        # Recall_after = (TP_after+TN_after) / GT_gland.size
                        # Acc_after = (TP_after+TN_after) / total_pseudo_af if total_pseudo_af != 0 else 0
                        #         # 将本次迭代的指标保存到字典中
...
def savezhibiao_to_file(snapshot_path,txtname,wuzhe_zhibiao):
        wuzhe_zhibiao = wuzhe_zhibiao
        file=open(os.path.join(snapshot_path,txtname),'w')
...
def main(bili_str,baifenbi):
    ...
    # snapshot_path=f'/mnt/hys/CPAC_Net/My_mt_with_reg_outputs_True/25%_lr_1e-2_伪标签由教师模型的确定标签与仿射伪标签共同构成_置信度阈值7e-1_20250408_consis_1e-1-11e-2_CE_threshold_6e-1_4e-1_变形图像也参与训练0.5'
    snapshot_path=f'/mnt/hys/CPAC_Net/My_mt_with_reg_outputs_True_33percent/33%_lr_1e-2_伪标签由教师模型的确定标签与仿射伪标签共同构成_置信度阈值95e-1_2025022_consis_1e-2-2e-2_CE_threshold_6e-1_4e-1_save_test_best_model_变形图像也参与训练0.5'

    # snapshot_path=f'/mnt/hys/CPAC_Net/My_mt_with_reg_outputs_True/25%_lr_1e-2_伪标签由教师模型的确定标签与仿射伪标签共同构成_置信度阈值7e-1_20250408_consis_1e-1-11e-2_CE_threshold_75e-2_25e-2'
    # snapshot_path=f'/mnt/hys/CPAC_Net/My_mt_with_reg_outputs_True/25%_lr_1e-2_伪标签由教师模型的确定标签与仿射伪标签共同构成_置信度阈值9e-1_20250408_consis_1e-1-11e-2_CE_threshold_75e-2_25e-2_变形图像也参与训练0.5'
    if not os.path.exists(snapshot_path):
        print("%s没有，正在创建" % snapshot_path)
        os.makedirs(snapshot_path)
    source1 = '/mnt/hys/CPAC_Net/train_my_mt_with_reg_同个病人_33percent.py'
    ...
    data_file_list=['/mnt/hys/Datasets/重复测量（hys整理版）/for_5fold/train_dict1.pkl','/mnt/hys/Datasets/重复测量（hys整理版）/for_5fold/train_dict2.pkl','/mnt/hys/Datasets/重复测量（hys整理版）/for_5fold/train_dict3.pkl','/mnt/hys/Datasets/重复测量（hys整理版）/for_5fold/train_dict4.pkl','/mnt/hys/Datasets/重复测量（hys整理版）/for_5fold/train_dict5.pkl']
    wuzhe_zhibiao=[]
    file=open(os.path.join(snapshot_path,'wuzhe.txt'),'w')
    a= 0
    wuzhe_sbnt_zhibiao=[] #see but no train
    wuzhe_ns_zhibiao=[] #no see
    for data_file in data_file_list:
        ...
        # if data_file=="data_new_2/train_dict5.pkl" or data_file=="data_new_2/train_dict1.pkl" or data_file=="data_new_2/train_dict2.pkl":
        train(snapshot_path,zhibiao_dir=dir_path,data_file=data_file,baifenbi=baifenbi,fold_num= a)
        # zhibiao_txt(path=os.path.join(dir_path, "zhibiao_unet_cross_pseudo_unaug_model1_best_resize_gai_contour_%s.txt" % (add_content)),
        #             dir_path=dir_path, data_path=f"{data_file}",img_path=img_path,
        #             model_path=os.path.join(snapshot_path, 'model1_best.pth'), centor_flag=centor_flag,
        #             pseudo_color=pseudo_color)
        # zhibiao_txt(path=os.path.join(dir_path, "zhibiao_unet_cross_pseudo_unaug_model2_best_resize_gai_contour_%s.txt" % (add_content)),
        #             dir_path=dir_path, data_path=f"{data_file}",img_path=img_path,
        #             model_path=os.path.join(snapshot_path, 'model2_best.pth'), centor_flag=centor_flag,
        #             pseudo_color=pseudo_color)
        ...
        test_sbnt_iou_2,test_ns_iou_2,test_iou_2=cal_zhibiao_hys_dantongdao_jiandanguiyihua_RCPS_33percent(fold_num=a,dir_path=dir_path,img_path=img_path,data_file=data_file,model_path=os.path.join(snapshot_path,data_file.split('/')[-1].split('.')[0],'model2_best.pth'),aux=False,data_type="test",device='gpu',model='unet',add_content=add_content,save_result_img=True)
        wuzhe_zhibiao.append(test_iou_2)
        wuzhe_sbnt_zhibiao.append(test_sbnt_iou_2)
        wuzhe_ns_zhibiao.append(test_ns_iou_2)
    

    savezhibiao_to_file(snapshot_path,'wuzhe_see_but_no_train_zhibiao.txt',wuzhe_sbnt_zhibiao)   
    savezhibiao_to_file(snapshot_path,'wuzhe_no_see_zhibiao.txt',wuzhe_ns_zhibiao)    
    savezhibiao_to_file(snapshot_path,'wuzhe_zhibiao.txt',wuzhe_zhibiao)   
