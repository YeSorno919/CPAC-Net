"""
    #25%标注比例下
    没有变形补充监督损失(DISS)、没有双源互补伪标签（DCPF），有强弱一致性（弱增强图像的教师预测与强增强的学生预测算ce损失）DRS-PIWAC
"""
import argparse
import logging
import os
...
def train(snapshot_path,zhibiao_dir,fold_num,data_file,epo_num=500,baifenbi=1,confidence_threshold=0.95):#bili用指百分之多少标注数据
    ...
    db_train= hysDataSets_dantongdao_jiandanguiyihua(data_file=data_file, img_path="/mnt/hys/Datasets/重复测量（hys整理版）/img_crop_npy", zhibiao_dir=zhibiao_dir,
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # noise = torch.clamp(torch.randn_like(unlabel_image) * 0.1, -0.2, 0.2)
            # ema_inputs = unlabel_image + noise
            with autocast():
                # 有标签 弱增强
                ...
                with torch.no_grad():
                    ...
                    unlabel_outputs2=model2(unlabel_image)["output"] #1,2 350 740 #预测的概率值
                    unlabel_outputs2_soft = torch.softmax(unlabel_outputs2, dim=1)
                # T = 8
                # _, _, w, h = unlabel_image.shape
                # volume_batch_r = unlabel_image.repeat(2, 1, 1, 1)
                ...
                #             (i + 1)] = model2(ema_inputs)["output"]
                # preds = F.softmax(preds, dim=1)
                
                preds = F.softmax(unlabel_outputs2, dim=1)
                # 无标签图像预测的不确定性
                uncertainty = -1.0 * \
                ...
                # consistency_dist = losses.softmax_mse_loss(unlabel_outputs1, ema_outputs)
                # consistency_dist = losses.softmax_mse_loss(strong_unlabel_image_outputs, unlabel_outputs2)
                ...
                consistency_dist = criterion(strong_unlabel_image_outputs,unlabel_outputs2_soft )

                if(epoch_num>10):
                    consistency_loss = torch.sum(
                    mask*consistency_dist)/(2*torch.sum(mask)+1e-16)
                else: consistency_loss=torch.tensor(0.0).to(device)
                loss_dice = dice_loss(
                ...
                #         # # consistence_loss = kl_loss(torch.log(unlabel_outputs_soft1), strong_unlabel_image_outputs_soft1)
                ...
                #     else:
                #         consistence_loss=torch.tensor(0.0).to(device)
                #         loss_consistence_dist.append(consistence_loss.detach().cpu())  
                supervised_loss_dist.append(supervised_loss.detach().cpu())  
                # UAMT的不确定性感知损失
                loss_pse_label_correction_dist.append(consistency_loss.detach().cpu())  
        ...
        writer.add_scalar('info/UAMT_uncertain_loss',
...
def main(bili_str,baifenbi):
    ...
    # snapshot_path=f'/mnt/hys/CPAC_Net/My_mt_with_reg_strong_weak_outputs/25%_lr_1e-2_伪标签由教师模型的确定标签与仿射伪标签共同构成_置信度阈值7e-1_20250408_consis_1e-1-11e-2_CE_threshold_6e-1_4e-1_变形图像也参与训练0.5'
    # snapshot_path=f'/mnt/hys/CPAC_Net/My_mt_with_reg_strong_weak_outputs/25%_lr_1e-2_伪标签由教师模型的确定标签与仿射伪标签共同构成_置信度阈值95e-1_20250408_consis_1e-1-11e-2_CE_threshold_6e-1_4e-1'
    # 没有rs和校正，单纯弱增强的教师预测和强增强的学生预测算ce（有不确定性）valbest
    snapshot_path=f'/mnt/hys/CPAC_Net/My_mt_with_reg_strong_weak_outputs/25%_lr_1e-3_500epoch_置信度阈值95e-2_20250415_consis_1e-2-11e-2_CE_s_w_threshold_6e-1_4e-1_变形图像也参与训练0.5_aug2_wo_pse_label_correct_and_rs_valbest'
    
    # snapshot_path=f'/mnt/hys/CPAC_Net/My_mt_with_reg_strong_weak_outputs/25%_lr_1e-3_500epoch_置信度阈值95e-2_20250414_consis_1e-2-11e-2_CE_s_w_threshold_75e-2_25e-2_变形图像也参与训练0.5_aug4_wo_pse_label_correct_and_rs'

    # snapshot_path=f'/mnt/hys/CPAC_Net/My_mt_with_reg_strong_weak_outputs/25%_lr_1e-3_500epoch_置信度阈值95e-2_20250412_consis_1e-1-11e-2_CE_s_w_threshold_75e-2_25e-2_变形图像也参与训练0.5_aug6'
    if not os.path.exists(snapshot_path):
        print("%s没有，正在创建" % snapshot_path)
        os.makedirs(snapshot_path)
    source1 = '/mnt/hys/CPAC_Net/train_my_mt_with_reg_同个病人_strong_weak_wo_reg_and_correct.py'
