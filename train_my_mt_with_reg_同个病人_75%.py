"""
    ...
    此代码为75%标注下，双源互补(DCPF)+变形图像补充监督(DISS)的实验 少了强弱增强一致性(DRS-PIWAC),注意没有进行数据增强，但有一致性
    若要消融DISS，修改损失即可
...
def main(bili_str,baifenbi):
    ...
    # snapshot_path=f'/mnt/hys/CPAC_Net/My_mt_with_reg_outputs_True_75%/20251226_lr_1e-2_伪标签由教师模型的确定标签与仿射伪标签共同构成_置信度阈值95e-2_consis_1e-2-2e-2_CE_threshold_6e-1_4e-1_变形图像也参与训练0.5'
    snapshot_path=f'/mnt/hys/CPAC_Net/My_mt_with_reg_outputs_True_75%/20251226_lr_1e-2_伪标签由教师模型的确定标签与仿射伪标签共同构成_置信度阈值95e-2_consis_1e-2-2e-2_CE_threshold_6e-1_4e-1'
    if not os.path.exists(snapshot_path):
        print("%s没有，正在创建" % snapshot_path)
        os.makedirs(snapshot_path)
    source1 = '/mnt/hys/CPAC_Net/train_my_mt_with_reg_同个病人_75%.py'
