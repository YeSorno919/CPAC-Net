        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num%len(trainloader)==0:
                ...
                with torch.no_grad():
                    ...
                    #     ogging.info("model2"+str(iou_val2))
                    ...
                    if avg_iou_test2 > best_performance2: #保存测试集合上最好的模型
                        ...
                        logging.info(f"计算并记录达到测试集上达到最优! Epoch: {best_performance_epoch}")
...
def main(bili_str,baifenbi):
    #没有引入配准监督损失/变形补充监督损失  和伪标签生成/矫正，而是消融（强弱一致性：弱增强图像的教师预测与强增强的学生预测算ce损失）
    snapshot_path=f'/mnt/hys/lf_codes/My_mt_with_reg_strong_weak_outputs_50%/20251226_lr_1e-3_500epoch_置信度阈值95e-2_consis_1e-2-11e-2_CE_s_w_threshold_6e-1_4e-1_变形图像也参与训练0.5_aug2_wo_pse_label_correct_and_rs_testbest'

    if not os.path.exists(snapshot_path):
        print("%s没有，正在创建" % snapshot_path)
        os.makedirs(snapshot_path)
    source1 = '/mnt/hys/lf_codes/train_my_mt_with_reg_同个病人_strong_weak_50%_wo_reg_and_correct.py'
    source2 = '/mnt/hys/lf_codes/mt_Dataset.py'
    source3 = '/mnt/hys/lf_codes/calzhibiao.py'
    source4 = '/mnt/hys/lf_codes/unet_model.py'
    source5 = '/mnt/hys/lf_codes/STN_2d.py'
    source6 = '/mnt/hys/lf_codes/Attention.py'
